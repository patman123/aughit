	...
	if(argc > 1)
		{
			...
			for( int i = 2; i < argc ; i++)
				{
					if(argv[i][0]>=51)
						FalseBricks.push_back(argv[i][0]-48);
					else 
						return -1;
				}
		}
	for(int i=0; i < FalseBricks.size(); i++)
		cout << FalseBricks[i] << endl;
	b2Body* floorBody;
	b2BodyDef floorDef;
			...
			while(1){
				...
				for (int i = 0; i < contours.size(); i++)
				{
					...
					if(blobs.size()==1)
					{
						...
						cout << "Center of Semi-Circular Paddle set to" << Threshx << endl;
					}
		  ...
		  pos != _contactListener->_contacts.end(); ++pos) {
			...
			if (bodyA->GetUserData() != NULL && bodyB->GetUserData() != NULL) {
				...
				bB=(int)bodyB->GetUserData();
			           ...
			           if (bA == 1 && bB > 2) {
			            	if (std::find(FalseBricks.begin(), FalseBricks.end(),bB)!= FalseBricks.end())
			            		destroyed[bB-3]=2;
			            	else if (std::find(toDestroy.begin(), toDestroy.end(), bodyB) == toDestroy.end()) {
			            ...
			            //Sprite A = block, Sprite B = ball
							...
							if (std::find(FalseBricks.begin(), FalseBricks.end(),bA )!= FalseBricks.end())
			            		destroyed[bA-3]=2;
			                else if (std::find(toDestroy.begin(), toDestroy.end(), bodyA) == toDestroy.end()) {
